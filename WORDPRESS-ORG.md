...
PMorix Post Type, Taxonomy & REST Generator is submitted as a complete ZIP whose top-level directory is
`pmorix-post-type-taxonomy-rest-generator`. The Composer production autoloader is included, so users do
...
5. Build the ZIP from a clean commit:
   ...
   `git archive --format=zip --prefix=pmorix-post-type-taxonomy-rest-generator/ --output=pmorix-post-type-taxonomy-rest-generator.zip HEAD`
